...
export class PlaywrightAdapter implements BrowserTaskExecutor {
  private cancelRequested = false;

  async runSearch(query: string, callbacks: BrowserTaskCallbacks): Promise<string> {
    callbacks.onStatus("Launching Playwright...");
    ...
    try {
      ...
      callbacks.onStatus(`Playwright navigating to ${url}`);
      ...
      callbacks.onStatus(`Playwright captured ${results.length} results`);
      return formatSearchResults("Playwright", results);
    } finally {
      await browser.close();
    }
  }
  ...
  async runFormFillDraft(
    ...
    if (!url) {
      throw new Error("Playwright form-fill draft requires an explicit URL in the request.");
    }
    ...
    try {
      ...
      const context = await browser.newContext();
      const page = await context.newPage();
      callbacks.onStatus(`Playwright opening form page ${url}`);
      await page.goto(url, { waitUntil: "domcontentloaded", timeout: SEARCH_TIMEOUT_MS });
      ...
      callbacks.onStatus(`Playwright drafted ${filledFields.length} fields`);
      return summarizeDraftFill("Playwright", url, filledFields);
    } finally {
      await browser.close();
    }
  }
...
async function loadPlaywright(): Promise<{
  ...
  try {
    return (await dynamicImportModule("playwright")) as {
      chromium: { launch: (options?: Record<string, unknown>) => Promise<any> };
    };
  } catch (error) {
    throw new Error(
      `Playwright is unavailable. Install it with 'npm i playwright'. ${error instanceof Error ? error.message : ""}`
    );
  }
}
